...
function Campaigns() {
    ...
    // Create form state
    const [form, setForm] = useState({ name: "", dmName: "", description: "", setting: "Forgotten Realms" });
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setForm({ name: "", dmName: "", description: "", setting: "Forgotten Realms" });
                setMessage("");
                loadCampaigns();
                setView("list");
            } else {
        ...
        } catch (err) { setMessage("Error creating campaign."); }
    };
    ...
    const handleUpdate = async () => {
        try {
            const res = await fetch(`http://localhost:8080/campaigns/${selected._id}`, {
                method: "PUT",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
    ...
    const handleAddNote = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:8080/campaigns/${selected._id}/notes`, {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            ...
            if (res.ok) {
                const newNote = { id: Date.now().toString(), text: noteInput, createdAt: new Date() };
                const updated = { ...selected, sessionNotes: [...(selected.sessionNotes || []), newNote] };
                setSelected(updated);
                setCampaigns((prev) => prev.map((c) => c._id === updated._id ? updated : c));
                setNoteInput("");
            }
    ...
    const openDetail = (campaign) => {
        ...
        setEditForm({ name: campaign.name, dmName: campaign.dmName, description: campaign.description, setting: campaign.setting });
        setEditing(false);
        setView("detail");
    };
            ...
            <main className="content">
                ...
                <div className="stats-row">
                    ...
                    <div className="stat-card">
                        <span className="stat-value">
                            {campaigns.reduce((acc, c) => acc + (c.sessionNotes?.length || 0), 0)}
                        </span>
                        <span className="stat-label">Session Notes</span>
                    </div>
                    <div className="stat-card">
                        <span className="stat-value">
                            {[...new Set(campaigns.map(c => c.setting).filter(Boolean))].length}
                        </span>
                ...
                {view === "list" && (
                    <>
                        <div className="section-header">
                            ...
                            <button className="btn-new" onClick={() => { setView("create"); setMessage(""); }}>
                                + New Campaign
                            </button>
                        </div>
                                ...
                                {campaigns.map((c) => (
                                    <div className="campaign-card" key={c._id}>
                                        <div className="campaign-card-top">
                                            <span className="campaign-setting-badge">{c.setting || "Homebrew"}</span>
                                            <h3 className="campaign-name">{c.name}</h3>
                                            <p className="campaign-dm">DM: {c.dmName || "Unknown"}</p>
                                        </div>
                                        <div className="card-divider" />
                                        <p className="campaign-desc">{c.description || "No description provided."}</p>
                                        <div className="campaign-meta">
                                            <span>📜 {c.sessionNotes?.length || 0} notes</span>
                                            <span>👥 {c.partyMembers?.length || 0} members</span>
                                        </div>
                                        <div className="card-footer">
                                            <button className="card-action" onClick={() => handleDelete(c._id)}>Delete</button>
                                            <button className="card-action primary" onClick={() => openDetail(c)}>Open →</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {view === "create" && (
                    <div className="form-panel">
                        ...
                        <div className="form-actions">
                            ...
                            <button className="btn-save" onClick={handleCreate}>Create Campaign</button>
                        </div>
                    </div>
                )}
                ...
                {view === "detail" && selected && (
                    <div className="detail-panel">
                        <button className="btn-back" onClick={() => setView("list")}>← Back to Campaigns</button>

                        <div className="detail-header">
                            <div>
                                <span className="campaign-setting-badge">{selected.setting || "Homebrew"}</span>
                                {editing ? (
                                    <input
                                ...
                                <p className="campaign-dm">DM: {editing
                                    ? <input className="inline-input" value={editForm.dmName} onChange={(e) => setEditForm({ ...editForm, dmName: e.target.value })} />
                            ...
                            <div className="detail-actions">
                                {editing ? (
                                    ...
                                    <button className="card-action primary" onClick={() => setEditing(true)}>Edit Campaign</button>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="detail-body">
                            ...
                            <section className="detail-section">
                                ...
                                {editing ? (
                                    ...
                                    <p className="detail-text">{selected.description || "No description provided."}</p>
                                )}
                            </section>
                            ...
                            <section className="detail-section">
                                ...
                                <div className="notes-list">
                                    {(selected.sessionNotes || []).length === 0 ? (
                                        <p className="empty-notes">No notes yet. Add your first session note below.</p>
                                    ) : (
                                ...
                                <div className="note-input-row">
                                    ...
                                    />
                                    <button className="btn-save" onClick={handleAddNote}>Add Note</button>
                                </div>
                            </section>
                        </div>
                    </div>
